...
const Hero = () => {
  ...
  return (
    <section 
      id="home" 
      className="min-h-screen relative flex items-center bg-temple-pattern"
      ref={heroRef}
    >
      {/* Background Overlay */}
      <div className="absolute inset-0 bg-sacred-gradient opacity-90"></div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white">
        <div className="hero-content">
          <h1 
            ref={titleRef}
            className="text-6xl md:text-8xl font-bold mb-6 font-display hero-title opacity-0"
          >
            Preserve Our
            <br />
            <span className="text-yellow-300">Sacred Heritage</span>
          </h1>
          ...
          >
            <button 
              ...
              className="btn-primary text-white px-10 py-4 rounded-full text-lg font-semibold transform hover:scale-105 transition-all duration-300 shadow-2xl"
            >
              🚀 Start Your Campaign
            </button>
            <button 
              onClick={() => scrollToSection('campaigns')}
              className="glass-effect text-white px-10 py-4 rounded-full text-lg font-semibold border-2 border-white hover:bg-white hover:text-orange-500 transition-all duration-300"
            >
              🔍 Explore Temples
            </button>
          </div>
